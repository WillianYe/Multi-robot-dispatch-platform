...
class FleetSocketIOBridge(Node):
    ...
    def robot_state_callback(self, msg: FleetState):
        for robot_state in msg.robots:
            try:
                if self.args.filter_fleet:
                    if self.args.filter_fleet not in robot_state.name:
                        return

                self._sio.emit(self.args.robot_state_topic,
                            message_to_ordereddict(robot_state))

                if self.args.gps_state_topic:
                    robot_state_json = self._robot_state_to_gps_json(robot_state)
