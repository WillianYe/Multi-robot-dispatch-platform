...
class RobotCommandHandle(adpt.RobotCommandHandle):
    ...
    def follow_new_path(
        ...
        if self.debug:
            plan_id = self.update_handle.unstable_current_plan_id()
            self.node.get_logger().info(
                f'follow_new_path for {self.name} with PlanId {plan_id}')
        self.interrupt()
        with self._lock:
            self._follow_path_thread = None
            ...
            def _follow_path():
                ...
                while self.remaining_waypoints \
                    ...
                    if self.state == RobotState.IDLE or target_pose is None:
                        ...
                        target_theta=[]
                        change_theta=False
                        for remaining_waypoint in self.remaining_waypoints:
                            if self.dist(self.position, remaining_waypoint.position) <0.1 and abs(remaining_waypoint.position[2]-self.position[2])<0.5:
                                continue
                            elif self.dist(self.position, remaining_waypoint.position) <0.1 and abs(remaining_waypoint.position[2]-self.position[2])>=0.5:
                                self.target_waypoint=remaining_waypoint
                                change_theta=True      
                                break
                            elif not target_point:
                                self.target_waypoint=remaining_waypoint
                                target_point.append(remaining_waypoint.position)
                                target_theta.append(math.atan2(
                                    remaining_waypoint.position[1]-self.position[1], remaining_waypoint.position[0]-self.position[0]))
                                continue
                            elif len(target_point)==1 and self.dist(target_point[0], remaining_waypoint.position) >=0.1:
                                target_point.append(remaining_waypoint.position)
                                target_theta.append(math.atan2(
                                    remaining_waypoint.position[1]-target_point[0][1], remaining_waypoint.position[0]-target_point[0][0]))
                                break
                        target_pose = self.target_waypoint.position
                        [x, y] =target_pose[:2]
                        theta = target_pose[2]
                        path_index = self.target_waypoint.index
                        if not change_theta:
                            if len(target_theta)==1:
                                theta=target_theta[0]
                            elif len(target_theta)==2:
                                dist1=self.dist(self.position,target_point[0])
                                dist2=self.dist(target_point[0],target_point[1])
                                tmp=dist1/(dist1+dist2)
                                theta=tmp*target_theta[0]+(1-tmp)*target_theta[1]
