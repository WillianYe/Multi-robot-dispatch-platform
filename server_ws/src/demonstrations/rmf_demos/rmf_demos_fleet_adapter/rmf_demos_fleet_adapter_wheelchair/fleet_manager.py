...
class FleetManager(Node):
    def __init__(self, config, nav_path):
        ...
        for robot_name, robot_config in self.config["robots"].items():
            ...
            self.init_state[robot_name] = False
        ...
        async def status(robot_name: Optional[str] = None):
            ...
            if robot_name is None:
                ...
                state = self.robots.get(robot_name)
                if state is None or state.state is None or self.init_state[robot_name]==False:
                    self.get_logger().info("robot name is not none but state is none")
                    return response
        ...
        async def navigate(robot_name: str, cmd_id: int, dest: Request):
            ...
            [cur_loc.x, cur_loc.y] = transforms["rmf_to_robot"].transform(
                [cur_loc.x, cur_loc.y])
            cur_loc.yaw += transforms['orientation_offset']
            path_request.path.append(cur_loc)

            [target_x, target_y] = transforms["rmf_to_robot"].transform(
                [dest.destination['x'], dest.destination['y']])
            target_yaw = copy.deepcopy(dest.destination['yaw'])
            target_yaw += transforms['orientation_offset']
            ...
            [robot.destination.x, robot.destination.y] = transforms['robot_to_rmf'].transform(
                [robot.destination.x, robot.destination.y])
            robot.destination.yaw -= transforms['orientation_offset']
            response['success'] = True
            return response
        ...
        async def start_process(robot_name: str, cmd_id: int, task: Request):
            ...
            for wp in self.docks[task.task]:
                ...
                [target_loc.x, target_loc.y] = transforms["rmf_to_robot"].transform([
                                                                                    target_loc.x, target_loc.y])
                target_loc.yaw += transforms['orientation_offset']
                path_request.path.append(target_loc)
        ...
        if (msg.name == self.fleet_name):
            for robot_state in msg.robots:
                if (robot_state.name in self.robots):
                    ...
                    [robot.state.location.x, robot.state.location.y] = self.transforms['robot_to_rmf'].transform(
                        [robot_state.location.x, robot_state.location.y])
                    robot.state.location.yaw -= self.transforms['orientation_offset']
                    self.init_state[robot_state.name]=True
                    ...
                    if (
                        ...
                        if robot.last_completed_request != completed_request and self.debug:
                            self.get_logger().info(
                    ...
                    self.get_logger().info(f"robot state name for : {robot_state.name} is not in self.robots")
