...
from rmf_fleet_msgs.msg import ModeRequest, PathRequest, Location, \
    FleetState, RobotMode, DockSummary, Dock, DockParameter
...
class MockDocker(Node):
    ...
    def __init__(self, config_yaml):
        ...
        self.robot_state_subscription = self.create_subscription(
            FleetState,'turtlebot3_fleet_states',self.fleet_state_cb, 10)
    ...
    def fleet_state_cb(self, msg: FleetState):
        for robot_state in msg.robots:
            ...
            requested_path = self.watching[robot_state.name]
            finish_location = requested_path.path[-1]
            if not close(finish_location, robot_state.location):
                return
            ...
            if robot_state.mode.mode != RobotMode.MODE_DOCKING:
